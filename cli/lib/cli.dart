...
Cell currentPlayer = Random().nextInt(2) + 1; // текущий игрок
...
bool expandBoard() {
  stdout.write('Насколько увеличить поле?: ');
  int? size = int.tryParse(stdin.readLineSync()!);
  size ??= 1;
  if (size + boardSize > 9) {
    return false;
  ...
  for(int k = 0; k < board.length; k++){
    List<int> buff = List.from(board[k]);
  ...
  for (int i = 0; i < size; i++){
    List<int> newBoard = List.filled(boardSize, empty);
    board.add(newBoard);
  }
...
void play() {
  while (state == playing) {
    ...
    if (state != playing) {
      stdout.write(
          'Если хотите реванш - введите y\nЕсли нет - введите q\nРасширить доску - введите e\n');
      while (true) {
        String? newgame = stdin.readLineSync();
        switch (newgame) {
          case 'y':
            cleanBoard();
            state = playing;
          ...
          case 'e':
            ...
            break;
          default:
            stdout.write('Введите корректную команду\n');
            continue;
        }
        break;
      }
